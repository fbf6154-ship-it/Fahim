...
CACHE = {
    "settings": {
        ...
        "support": "☎️ <b>হেল্প ও সাপোর্ট ডেস্ক</b>\n━━━━━━━━━━━━━━━━━━━━━\nযেকোনো সমস্যা বা উইথড্র সংক্রান্ত সহায়তার জন্য এডমিনকে সরাসরি মেসেজ দিন:\n\n👤 <b>এডমিন:</b> @Promoter_from_bd\n📢 <b>অফিশিয়াল চ্যানেল:</b> @FHx_Technical_Creator",
        "min_withdraw": 15.0,
        "max_withdraw": 100.0,
        "refer_bonus": 2.0,
        "daily_bonus": 0.1,
...
# --- 🌐 FLASK KEEP-ALIVE SERVER (24/7 অন রাখার জন্য) ---
app = Flask(__name__)
...
# --- ⌨️ মেইন মেনু কিবোর্ড (সম্পূর্ণ বাংলা) ---
def main_menu():
...
def start_cmd(message):
    try:
        ...
        if settings.get("maintenance") and user_id != ADMIN_ID:
            bot.send_message(user_id, "🛠️ <b>বটটিতে বর্তমানে মেইনটেনেন্স চলছে। অনুগ্রহ করে কিছুক্ষণ পর আবার চেষ্টা করুন!</b>")
            return
        ...
        if user_data.get("status") == "banned":
            bot.send_message(user_id, "🚫 <b>আপনাকে এই বটটি ব্যবহার করা থেকে নিষিদ্ধ করা হয়েছে!</b>")
            return
        ...
        if is_joined(user_id):
            bot.send_message(user_id, "🎉 <b>স্বাগতম! নিচের মেনু থেকে অপশন বেছে নিন:</b>", reply_markup=main_menu())
            return
...
def verify_callback(call):
    ...
    if is_joined(user_id):
        ...
        currency = CACHE["settings"].get("currency", "টাকা")
        ...
        if referrer_id and not user_data.get("bonus_claimed"):
            ...
            if ref_data:
                ...
                try:
                    bot.send_message(referrer_id, f"🎉 <b>অভিনন্দন! আপনার রেফার লিংকের মাধ্যমে নতুন ইউজার যুক্ত হওয়ায় আপনি +{refer_bonus} {currency} পেয়েছেন!</b>")
                except:
                    pass
        ...
        try:
            ...
            pass
        bot.send_message(user_id, "🎉 <b>মেম্বারশিপ সফলভাবে যাচাই হয়েছে! স্বাগতম।</b>", reply_markup=main_menu())
    else:
        bot.answer_callback_query(call.id, "❌ আপনি এখনো সবগুলো চ্যানেলে জয়েন করেননি! জয়েন করে আবার চেষ্টা করুন।", show_alert=True)
...
def balance_handler(message):
    ...
    wallet = user_data.get("wallet", "সেট করা নেই")
...
def referral_handler(message):
    ...
    markup = types.InlineKeyboardMarkup()
    share_url = f"https://t.me/share/url?url={ref_link}&text=🚀 এই বট থেকে প্রতি রেফারে {ref_bonus} {currency} ফ্রিতে আয় করুন!"
...
━━━━━━━━━━━━━━━━━━━━━
<i>বন্ধুদের সাথে লিংক শেয়ার করে আনলিমিটেড বোনাস আয় করুন!</i>"""
...
def bonus_handler(message):
    ...
    user_data = get_user(user_id)
    settings = get_settings()
    currency = settings.get("currency", "টাকা")
    ...
    if current_time - last_time >= cooldown_seconds:
        ...
        msg = f"""🎁 <b>ডেইলি বোনাস গ্রহণ সম্পন্ন!</b>
━━━━━━━━━━━━━━━━━━━━━
🎉 অভিনন্দন! আপনি সফলভাবে আজকের বোনাস পেয়েছেন।

💰 <b>বোনাস পেয়েছেন:</b> +{bonus_amt} {currency}
🕒 <b>{cooldown_hours} ঘণ্টা</b> পর পুনরায় বোনাস নিতে পারবেন।
━━━━━━━━━━━━━━━━━━━━━"""
        bot.send_message(user_id, msg)
    else:
        ...
        msg = f"""⏳ <b>আজকের বোনাস ইতিমধ্যে নেওয়া হয়েছে!</b>
━━━━━━━━━━━━━━━━━━━━━
...
def withdraw_init(message):
    ...
    if not settings.get("withdraw_status", True):
        bot.send_message(user_id, "⚠️ <b>উইথড্র সিস্টেম সাময়িকভাবে বন্ধ আছে।</b>")
        return
    ...
    if wallet in ["সেট করা নেই", "Not Set"]:
        bot.send_message(user_id, "⚠️ <b>আপনার ওয়ালেট নম্বর সেট করা নেই!</b>\nআগে 💳 <b>ওয়ালেট</b> বাটনে চাপ দিয়ে বিকাশ/নগদ নম্বর সেট করুন।")
        return

    if balance < min_withdraw:
        bot.send_message(user_id, f"❌ <b>উইথড্র করার পর্যাপ্ত ব্যালেন্স নেই!</b>\nসর্বনিম্ন উইথড্র: <b>{min_withdraw} {currency}</b>\nআপনার ব্যালেন্স: <b>{balance:.2f} {currency}</b>")
        return
...
def process_withdrawal(message, balance, wallet, min_withdraw, currency):
    ...
    try:
        ...
        bot.send_message(user_id, "⚠️ ভুল ইনপুট! শুধুমাত্র সংখ্যার পরিমাণ লিখুন।")
        return
    ...
    admin_markup.add(
        types.InlineKeyboardButton("✅ পেমেন্ট সম্পন্ন", callback_data=f"appr_{w_id}"),
        types.InlineKeyboardButton("❌ বাতিল ও রিফান্ড", callback_data=f"rej_{w_id}")
    )
    ...
    bot.send_message(user_id, f"উইথড্র রিকোয়েস্ট সফল হয়েছে ✅\n\n💰 <b>পরিমাণ:</b> {amount} {currency}\n⏳ <b>পেমেন্ট স্ট্যাটাস:</b> পেন্ডিং (Pending)\n💳 <b>ওয়ালেট:</b> <code>{wallet}</code>\n🔖 <b>আইডি:</b> <code>{w_id}</code>\n\n<i>এডমিন শীঘ্রই আপনার পেমেন্ট পাঠিয়ে ভেরিফাই করে দিবে।</i>")
...
def admin_withdraw_decision(call):
    ...
    if action == "appr":
        ...
        bot.edit_message_text(f"{call.message.text}\n\n✅ <b>স্ট্যাটাস: পেইড ও অনুমোদিত (APPROVED)</b>", call.message.chat.id, call.message.message_id)
        try:
            bot.send_message(user_id, f"🎉 <b>আপনার {amount} {currency} উইথড্র সফল হয়েছে এবং টাকা পাঠিয়ে দেওয়া হয়েছে!</b>\n💳 ওয়ালেট: <code>{wallet}</code>")
        except:
            pass
...
def admin_main_menu_markup():
    ...
    markup.add(
        types.InlineKeyboardButton(f"🛠️ মেইনটেনেন্স: {maint_status}", callback_data="adm_toggle_maint"),
        types.InlineKeyboardButton(f"📥 উইথড্র সিস্টেম: {wd_status}", callback_data="adm_toggle_wd")
    )
    markup.add(
    ...
    markup.add(
        types.InlineKeyboardButton(f"💱 কারেন্সি ({settings.get('currency')})", callback_data="adm_set_curr"),
        types.InlineKeyboardButton(f"👥 প্রতি রেফার ({settings.get('refer_bonus')})", callback_data="adm_set_refb")
    )
    markup.add(
        types.InlineKeyboardButton(f"🔻 সর্বনিম্ন উইথড্র ({settings.get('min_withdraw')})", callback_data="adm_set_minw"),
        types.InlineKeyboardButton(f"🎁 ডেইলি বোনাস ({settings.get('daily_bonus')})", callback_data="adm_set_dailyb")
    )
    markup.add(
        types.InlineKeyboardButton("🖼️ ব্যানার ইমেজ পরিবর্তন", callback_data="adm_set_photo_prompt"),
...
def admin_panel_open(message):
    ...
    bot.send_message(
        ADMIN_ID,
        "👑 <b>ইন্টারঅ্যাক্টিভ এডমিন কন্ট্রোল প্যানেল</b>\n━━━━━━━━━━━━━━━━━━━━━\nনিচের বোতামগুলো ব্যবহার করে বট নিয়ন্ত্রণ করুন:",
...
def admin_callbacks(call):
    if str(call.from_user.id) != ADMIN_ID:
        bot.answer_callback_query(call.id, "❌ অ্যাক্সেস ডিনাইড!")
        return
    ...
    if data == "adm_back_main":
        ...
        bot.edit_message_text(
            f"📢 <b>চ্যানেল ম্যানেজার:</b>\n━━━━━━━━━━━━━━━━━━━━━\nমোট যুক্ত চ্যানেল: <b>{len(channels)}</b> টি\nচ্যানেল ডিলিট বা বিস্তারিত দেখতে নামের ওপর চাপুন:",
            call.message.chat.id,
            call.message.message_id,
            reply_markup=markup
        )
        ...
        markup.add(types.InlineKeyboardButton("🔙 চ্যানেল লিস্টে ফিরুন", callback_data="adm_channel_mgr"))

        msg_text = f"📢 <b>চ্যানেলের বিস্তারিত:</b>\n━━━━━━━━━━━━━━━━━━━━━\n🔹 <b>নাম:</b> {ch_info.get('title')}\n🔹 <b>আইডি:</b> <code>{ch_info.get('channel_id')}</code>\n🔹 <b>লিংক:</b> {ch_info.get('url')}"
        ...
        msg = bot.send_message(ADMIN_ID, "👥 <b>প্রতি রেফারের বোনাস পরিমাণ লিখুন (যেমন: 2):</b>")
        ...
        bot.register_next_step_handler(msg, lambda m: [update_settings({"min_withdraw": float(m.text.strip())}), bot.send_message(ADMIN_ID, f"✅ মিনিমাম উইথড্র লিমিট আপডেট হয়েছে!")])
        ...
        current_img = settings.get("photo", "কোনো ছবি সেট নেই")
        msg = bot.send_message(ADMIN_ID, f"🖼️ <b>নতুন ব্যানার ইমেজের সরাসরি (Direct) URL লিংক পাঠান:</b>\n\n📌 <b>বর্তমান লিংক:</b>\n<code>{current_img}</code>\n\n<i>(নতুন লিংক পেস্ট করে পাঠান)</i>")
        bot.register_next_step_handler(msg, process_set_photo_step)
        ...
        msg = bot.send_message(ADMIN_ID, "📢 <b>সকল ইউজারের কাছে পাঠানোর মেসেজটি পাঠান:</b>")
        bot.register_next_step_handler(msg, do_broadcast_step)

# --- 👑 এডমিন স্টেপ হ্যান্ডলারস ---
def process_set_photo_step(message):
    photo_url = message.text.strip()
    if photo_url.startswith("http://") or photo_url.startswith("https://"):
        update_settings({"photo": photo_url})
        bot.send_message(ADMIN_ID, f"✅ <b>ব্যানার ফটো সফলভাবে আপডেট করা হয়েছে!</b>\n\n🔗 {photo_url}", reply_markup=admin_main_menu_markup())
    else:
        bot.send_message(ADMIN_ID, "⚠️ ভুল লিংক! অনুগ্রহ করে http:// বা https:// সহ সঠিক ডাইরেক্ট ইমেজ লিংক দিন।")
...
def process_add_channel_step(message):
    try:
        ...
        bot.send_message(ADMIN_ID, "⚠️ ভুল ফরম্যাট! দয়া করে সঠিক নিয়মে পাঠান।")
...
💳 <b>ওয়ালেট:</b> <code>{u.get('wallet', 'সেট করা নেই')}</code>
...
def do_broadcast_step(message):
    ...
    for uid in users:
        try:
            ...
            pass
    bot.send_message(ADMIN_ID, f"📣 <b>ব্রডকাস্ট সফল হয়েছে! মোট {count} জন ইউজারের কাছে মেসেজ পৌঁছেছে।</b>")
